...
function isAuthorized(key: string | null) {
  return key === process.env.PRODUCTION_SECRET || key === process.env.ADMIN_SECRET;
}
...
export async function GET(req: Request) {
  ...
  const { data, error } = department ? await query.eq("department", department) : await query;
  if (error) {
...
export async function POST(req: Request) {
  ...
  const { error } = await supabaseAdmin.from("needs").insert({ department, content });
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
